...
const DAYS = [
    {
        date: '18 May', weekday: 'Lunes', theme: 'Apertura', color: '#F7931A',
        events: [
    ...
    {
        date: '19 May', weekday: 'Martes', theme: 'Tecnología', color: '#c47314',
        events: [
    ...
    {
        date: '20 May', weekday: 'Miércoles', theme: 'Economía', color: '#F7931A',
        events: [
    ...
    {
        date: '21 May', weekday: 'Jueves', theme: 'Comunidad', color: '#c47314',
        events: [
    ...
    {
        date: '22 May', weekday: 'Viernes', theme: 'Comunidad', color: '#c47314',
        events: [
            { time: '10:00', title: 'Encuentro de comunidades', type: 'Networking', venue: 'Múltiples sedes', speaker: 'Comunidades Bitcoin UY', desc: 'Espacio libre para que las distintas comunidades del ecosistema se encuentren.' },
            { time: '14:00', title: 'Bitcoin para el público general', type: 'Charla', venue: 'Sede comunitaria · TBD', speaker: 'TBD', desc: 'Introducción accesible a Bitcoin sin tecnicismos.' },
            { time: '18:00', title: 'Social: Bitcoin Pizza Night', type: 'Social', venue: 'TBD', speaker: '', desc: 'Celebración del Bitcoin Pizza Day con pizza y conversación.' },
    ...
    {
        date: '23 May', weekday: 'Sábado', theme: 'Comunidad', color: '#c47314',
        events: [
            { time: '10:00', title: 'Encuentro de comunidades', type: 'Networking', venue: 'Múltiples sedes', speaker: 'Comunidades Bitcoin UY', desc: 'Espacio libre para que las distintas comunidades del ecosistema se encuentren.' },
            { time: '14:00', title: 'Bitcoin para el público general', type: 'Charla', venue: 'Sede comunitaria · TBD', speaker: 'TBD', desc: 'Introducción accesible a Bitcoin sin tecnicismos.' },
            { time: '18:00', title: 'Social: Bitcoin Pizza Night', type: 'Social', venue: 'TBD', speaker: '', desc: 'Celebración del Bitcoin Pizza Day con pizza y conversación.' },
    ...
    {
        date: '24 May', weekday: 'Domingo', theme: 'Bitcoin Pizza Day', color: '#F7931A',
        events: [
...
const TYPE_COLOR = {
    Keynote: '#F7931A', Panel: '#7B61FF', Workshop: '#22C55E',
    Charla: '#3B9EFF', Networking: '#F97316', Social: '#EC4899', Cierre: '#F7931A',
}
    ...
    return (
        <main style={{ paddingTop: 'var(--nav-h)' }}>
            ...
            <section style={{ background: 'var(--black)', padding: '80px 0 56px', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 55% 80% at 0% 50%, rgba(247,147,26,0.06) 0%, transparent 60%)' }} />
                    ...
                    <FadeIn delay={0.1}>
                        <div style={{ display: 'flex', gap: 20, marginTop: 28, flexWrap: 'wrap' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: 'var(--text-secondary)', fontSize: 12 }}>
                                <Calendar size={12} style={{ color: 'var(--btc)' }} /> 18–22 Mayo 2026
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: 'var(--text-secondary)', fontSize: 12 }}>
                                <MapPin size={12} style={{ color: 'var(--btc)' }} /> Montevideo, Uruguay
                            </div>
                        </div>
                    </FadeIn>
                </div>
                    ...
                    <div style={{ display: 'flex', overflowX: 'auto' }}>
                        {DAYS.map((day, i) => (
                            <button key={i} onClick={() => setActiveDay(i)}
                                style={{
                                    padding: '18px 26px', fontFamily: 'var(--font-body)', fontSize: 11,
                                    fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap',
                            ...
                            >
                                <span style={{ display: 'block', fontSize: 9, fontFamily: 'var(--font-mono)', opacity: 0.55, marginBottom: 2, letterSpacing: '0.15em' }}>{day.weekday}</span>
                                {day.date}
                            </button>
                        ))}
                    </div>
            ...
            <section className="section" style={{ background: 'var(--black)' }}>
                <div className="container">
                    ...
                    >
                        {/* Day header */}
                        <div style={{ marginBottom: 44, borderLeft: `3px solid ${DAYS[activeDay].color}`, paddingLeft: 22 }}>
                            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(38px,6vw,60px)', color: 'var(--text-primary)', letterSpacing: '0.03em', lineHeight: 0.95 }}>
                                {DAYS[activeDay].date} · {DAYS[activeDay].weekday}
                            </h2>
                            <p style={{ color: 'var(--btc)', fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: 6 }}>
                                ...
                                return (
                                    <motion.div key={i}
                                        ...
                                        style={{
                                            ...
                                            borderLeft: open ? `3px solid ${tc}` : '1px solid var(--border)',
                                        }}
                                        onMouseEnter={e => e.currentTarget.style.borderColor = open ? tc : 'var(--border-hot)'}
                                        onMouseLeave={e => e.currentTarget.style.borderColor = open ? tc : 'var(--border)'}
                                    >
                                        {/* Row */}
                                        <div style={{ padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                                            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, color: 'var(--btc)', minWidth: 56, fontWeight: 500 }}>{ev.time}</div>
                                            <div style={{ flex: 1 }}>
                                                <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 15, color: 'var(--text-primary)', marginBottom: 5 }}>{ev.title}</h3>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                                                    <span style={{ fontSize: 10, padding: '2px 10px', borderRadius: 999, background: `${tc}18`, color: tc, fontFamily: 'var(--font-mono)', letterSpacing: '0.1em' }}>
                                                        {ev.type}
                                                    </span>
                                                    {ev.venue && (
                                        ...
                                        <AnimatedExpand open={open}>
                                            <div style={{ padding: '0 22px 18px', paddingLeft: 98, borderTop: '1px solid var(--border)' }}>
                                                <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.75, marginTop: 14 }}>{ev.desc}</p>
                                                {ev.speaker && (
                                                    <p style={{ marginTop: 10, color: 'var(--text-muted)', fontSize: 11, fontFamily: 'var(--font-mono)', display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <section style={{ background: 'var(--surface)', padding: '56px 0', borderTop: '1px solid var(--border)', textAlign: 'center' }}>
                <div className="container">
                    <FadeIn>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginTop: 10, lineHeight: 1.7 }}>
